...
import { getParamByParam,getAllISOCodes } from 'iso-country-currency'
...
export class AddQuotation extends Component {
    ...
    componentDidMount() {
        ...
        .then(res=>{
            ...
            let currency=''
            if(findcountry.name==='Viet Nam'){
            currency = getParamByParam('countryName','Vietnam','currency')
            }else{
            currency = getParamByParam('countryName',findcountry.name,'currency')
            }
    ...
    if (this.state.value !== undefined) {
        this.notification(this.props.id)
         const quote_detail =this.state.value
    ...
    .then(res => {
        ...
        if (this.props.id !== undefined) {
           this.notification(this.props.id)
            ...
            axiosInstance.post(`/quotation/activity/${this.props.id}`)
    ...
    axiosInstance.post(`/quotation/search/${localStorage.getItem('countryid')}`)
    .then(res=>{
        const quotation_list2 = res.data.response.quotation_list
        // console.log(quotation_list2,'list-E')
        // console.log(this.props.id,'ide')
        this.setState({quotation_list2})
        // if (this.props.id !== undefined) {
        // this.setState({parent_QuoteNum: quotation_list2 !== undefined ? quotation_list2.quotation_num : '--parent quote--'})
        if (this.props.id !== undefined) {
            const quote_detail = quotation_list2.find(quote => parseInt(quote.id) === parseInt(this.props.id))
            // console.log(quote_detail,'imabad')
            const parent_quote = quotation_list2.find(quote => parseInt(quote.id) === parseInt(quote_detail.parent_id))
            // console.log(parent_quote,'imabad2')
            this.setState({
                // parent_id: quote_detail.id,
                parent_QuoteNum: parent_quote !== undefined ? parent_quote.quotation_num : '--parent quote--'
            })
        }
    ...
    componentWillReceiveProps({value}) {
        ...
        // console.log('received',this.state.value)  
      }
    ...
    render() {
        const { quote_id, clientName, parent_id, categories, quotation_num, quotationDate, client_id, quoteAmount, description, quotationStatus, client_po, margin, margin_amount,
            ...
            client_amount, cont_amount, currency,user,vertical,horizontal,showSnak,rerender, comments2 ,quote_comments,quotation_list2} = this.state;
        ...
        return (
            <div>
                ...
                <div className="component">
                    <Row>
                    <Col lg={4} sm={4} md={4} xs={12} className='d-none d-md-block'>
                    {this.props.id !== undefined ? <button className='button' primary onClick={this.props.Back} >Back</button>:
                        <button className='button' primary onClick={()=>window.history.back()} >Back</button>}
                    </Col>
                        {/* <Col lg={2}>
                        {quote_id === undefined ?
                        ...
                        <Col className='d-flex justify-content-end' lg={8} sm={8} md={8} xs={12}>
                            ...
                            <div style={{ margin: '0px 5px' }}>
                                <Autocomplete
                                    options={quotation_list2 !== undefined && quotation_list2.filter(quote => quote.parent_id === '0' && quote.id !== this.props.id)}
                                    // onChange={(e, value) => value !== null ? this.setState({ parent_id: value.id, ticket_no: value.ticket_no }) : this.setState({ parent_id: '' })}
                                    onChange={(e, value) => value !== null ? this.setState({ parent_id: value.id}) : this.setState({ parent_id: '' })}
                   ...
                   <Col  className='d-block d-md-none'>
                    {this.props.id !== undefined ? <button style={{width:70}} className='button' primary onClick={this.props.Back} >Back</button>:
                        <button className='button' style={{width:60}} primary onClick={()=>window.history.back()} >Back</button>}
                    </Col>
                       <Col lg={3}>
                      ...
                      <hr/>
                        ...
                        <Row>
                            ...
                            <Col lg={1} >
                                {margin_amount >= 0 ? <CustomTextBox
                                    ...
                                    txtBoxType="text"
                                    txtBoxName="margin"
                                    txtBoxValue={quotationStatus=='1'?`${margin}%`:''}
                                    txtBoxPH="%"
                                    disabled="true"
                                /> :
                                    <CustomTextBox
                            ...
                            <Col lg={2} >
                                {margin_amount >= 0 ?
                                    <CurrencyFormat
                                        className='form-control'
                                        placeholder="Amount"
                                        value={quotationStatus=='1'?margin_amount:''}
                                        thousandSeparator={true}
                                        disabled="true"
                        ...
                        <Row>
                            ...
                            <Col lg={3}>
                                {/* <CustomTextBox
                                    ...
                                    txtBoxLabel="Job Completion Date"
                                    txtBoxType="date"
                                ...
                                /> */}
                                <Form.Group >
                                <Form.Label>Job Completion Date</Form.Label>
                                 <Form.Control
                                        id='jobComplete'
                                        type="date"
                                        name="jobComplete"
                            ...
                            <Col lg={3}>
                                <Form.Group>
                                <Form.Label>Overall Status</Form.Label>
                       ...
                       <Row>
                            ...
                            <Col lg={2}>
                            {quote_id !== undefined
                            ?  <button className='button'  onClick={this.onEdit}>Modify</button>
                            : ''}
                            </Col>
                        </Row>                     
                    </Card>}
                    ...
                    />}
                   <Card className='d-none d-md-block' style={{ marginTop: '15px'}}>
                    <Row>
                        ...
                        <Col lg={3} sm={3} md={3} xs={12}  style={{marginTop:101}}>
                        <CustomTextBox
                                    txtBoxID='comments2'
                                    // txtBoxLabel="usrname"
                                    txtBoxType="text"
                        ...
                        <Col lg={1} sm={1} md={1} xs={12}  style={{marginTop:140}}> 
                        <button className='button'  onClick={this.AddComment}>Send</button>
                    ...
                    <Card className='d-block d-md-none' style={{ marginTop: '15px'}}>
                    <Row>
                        <Col lg='6' sm={6} md={6} xs={12}>
                        <div class="form-group">
                            <Form.Label >Comments</Form.Label>
                    ...
                    <Row  style={{ marginTop: '20px' }}>
                        {/* {this.props.id !== undefined ? <CustomButton btnType="reset" BtnTxt="Back" ClickEvent={this.props.Back} />:
